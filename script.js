...
async function GetSongs(folder) {
  ...
  let songURL = document.querySelector(".song-list").getElementsByTagName("ul")[0];
  songURL.innerHTML = "";
...
async function getFolder(){
  ...
  for(let i=0; i<array.length; i++){
    ...
    if (e.href.includes('/song/')) {
      ...
      const response = await fetch(`http://127.0.0.1:5500/song/${folderName}/info.json`);
...
////add eventlistner to volume update
  let volumeVar=document.querySelector(".range").getElementsByTagName("input")[0].addEventListener("change", (e) => {
